...
<div id="app">
      ...
      <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
        <li class="nav-item">
            <? if (User_model::is_logged()) {?>
              <a href="/main_page/logout" class="btn btn-primary my-2 my-sm-0" @click.prevent="logout" >Log out, <?= $user->personaname?>
              </a>
            <? } else {?>
  ...
  <div class="modal fade" id="postModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" v-if="post">
    <div class="modal-dialog modal-xl" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="postModalLabel">Post {{post.id}}</h5>
          ...
          <div class="card mb-3">
            ...
            <div class="card-body">
              <div class="likes" @click="addLike('post', post.id)">
                ...
                <!-- <li class="node-tree">
                  <span class="label">{{ comments_tree_data.user.personaname + \' - \'}}</span>
                  <small class="text-muted">{{comments_tree_data.text}}</small>
                  <a role="button" @click="likeComment(\'comment\', comments_tree_data.id)">
                    <svg class="bi bi-heart-fill" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                      <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" clip-rule="evenodd"/>
                    </svg>{{ comments_tree_data.likes }}    -   {{ comments_tree_data.time_created }}
                  </a>
                  <br>
                  <div class="addRelpyComment " :id="comment_id_{{ comments_tree_data.id }}">
                  ...
                  <ul v-if="comments_tree_data.children && comments_tree_data.children.length">
                    <comments_tree_node v-for="child_comment in comments_tree_data.children" :comments_tree_data="child_comment">
